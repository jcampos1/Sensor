...
angular.module("GRIDMAE1016", [ 'ui.bootstrap', 'ngTouch', 'ui.grid',
		'ui.grid.pagination', 'ui.grid.selection', 'messages' ]);

/* *********************** SERVICIOS ********************** */

"use strict";
angular.module('GRIDMAE1016').service('MAE1016ConfigurationGrid',
		MAE1016ConfigurationGrid);
MAE1016ConfigurationGrid.$inject = [ '$http', '$log', '$uibModal',
		'uiGridConstants', '$translate' ];
function MAE1016ConfigurationGrid($http, $log, $uibModal,
		uiGridConstants, $translate) {
	/** ********************** VARIABLES PRIVADAS ******************* */
	...
	return {
		initializeGridOptions : function($scope, $uibModalInstance) {
			...
			$scope.entitySelect = function(row) {
				...
				$scope.select_mae1016 = row.entity;
				$uibModalInstance.close($scope.select_mae1016);
			};
	...
	function getPage($scope) {
		...
		$http.post("/WeighBridgeStandAlone/MAE1016/externPagSimple", obj).then(function(response) {
