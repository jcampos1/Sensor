...
angular.module("GRIDMAE1017", [ 'ui.bootstrap', 'ngTouch', 'ui.grid',
		'ui.grid.pagination', 'ui.grid.selection', 'messages' ]);

/* *********************** SERVICIOS ********************** */

"use strict";
angular.module('GRIDMAE1017').service('MAE1017ConfigurationGrid',
		MAE1017ConfigurationGrid);
MAE1017ConfigurationGrid.$inject = [ '$http', '$log', '$uibModal',
		'uiGridConstants', '$translate' ];
function MAE1017ConfigurationGrid($http, $log, $uibModal,
		uiGridConstants, $translate) {
	/** ********************** VARIABLES PRIVADAS ******************* */
	...
	return {
		initializeGridOptions : function($scope, $uibModalInstance) {
			...
			$scope.entitySelect = function(row) {
				$scope.select_mae1017 = row.entity;
				$uibModalInstance.close($scope.select_mae1017);
			};
	...
	function getPage($scope) {
		...
		$http.post("/WeighBridgeStandAlone/MAE1017/externPagSimple", obj).then(function(response) {
