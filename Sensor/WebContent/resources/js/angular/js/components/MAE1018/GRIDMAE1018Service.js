...
angular.module("GRIDMAE1018", [ 'ui.bootstrap', 'ngTouch', 'ui.grid',
		'ui.grid.pagination', 'ui.grid.selection', 'messages' ]);

/* *********************** SERVICIOS ********************** */

"use strict";
angular.module('GRIDMAE1018').service('MAE1018ConfigurationGrid',
		MAE1018ConfigurationGrid);
MAE1018ConfigurationGrid.$inject = [ '$http', '$log', '$uibModal',
		'uiGridConstants', '$translate' ];
function MAE1018ConfigurationGrid($http, $log, $uibModal,
		uiGridConstants, $translate) {
	/** ********************** VARIABLES PRIVADAS ******************* */
	...
	return {
		initializeGridOptions : function($scope, $uibModalInstance) {
			...
			$scope.entitySelect = function(row) {
				$scope.select_mae1018 = row.entity;
				$uibModalInstance.close($scope.select_mae1018);
			};
	...
	function getPage($scope) {
		...
		$http.post("/WeighBridgeStandAlone/MAE1018/externPagSimple", obj).then(function(response) {
